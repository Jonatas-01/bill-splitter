...
The main page component orchestrates the full bill-splitting flow by switching between each step in the process. It is responsible for:

- starting the upload flow,
- moving through bill review and people setup,
- managing dish assignment,
- showing the final bill summary, and
- keeping shared state available across steps.

### Local State

| State | Type | Initial value | Description |
|-------|------|---------------|-------------|
| `currentView` | `View` | `"upload"` | Controls which step is visible on screen. |
| `billItems` | `ExtractedBill \| null` | `null` | Stores the current bill data shared between later steps. |
| `people` | `Person[]` | `[]` | Stores the participant list used for assignment and summary screens. |
| `finalBill` | `FinalBill \| null` | `null` | Stores the final bill produced by the summary step. |

### View Flow

The page uses a `View` union type with these steps:
...
1. `upload` -> `review` after a successful bill extraction.
2. `review` -> `addPeople` after the reviewed bill is confirmed.
3. `addPeople` -> `assignDishes` after at least two people are collected.
4. `assignDishes` -> `billSummary` after all assignment rules pass.
5. `billSummary` -> completion when the final bill is confirmed.

### Step Handlers

#### Upload Step

- Renders `UploadPhoto` when `currentView === "upload"`.
- `onSuccess` stores the extracted bill in `billItems` and switches to `review`.

#### Review Step

- Renders `ReviewBill` when `currentView === "review"` and `billItems` exists.
- `onConfirm` updates the bill with reviewed edits and moves to `addPeople`.
- `onTryAgain` clears the bill and returns to `upload`.

#### Add People Step

- Renders `AddPeople` when `currentView === "addPeople"`.
- `onBack` returns to `review`.
- `onNext` stores the updated people list and moves to `assignDishes`.

#### Assign Dishes Step

- Renders `AssignDishes` when `currentView === "assignDishes"`, `billItems` exists, and `people.length > 0`.
- `onUpdateBill` persists item assignment changes back into `billItems`.
- `onBack` returns to `addPeople`.
- `onNext` advances to `billSummary`.

#### Bill Summary Step

- Renders `BillSummary` when `currentView === "billSummary"`, `billItems` exists, and `people.length > 0`.
- `onBack` returns to `assignDishes`.
- `onNext` stores the final bill in `finalBill`, shows a completion alert, and logs the final bill to the console.

### Data Flow

- `billItems` is the shared bill state passed through review, assignment, and summary steps.
- `people` is collected once in the people step and reused in later steps.
- `finalBill` is only set after the summary confirmation step.

### Render Conditions

- The page only renders one step at a time.
- `review`, `assignDishes`, and `billSummary` all require a non-null `billItems` value.
- `assignDishes` and `billSummary` also require at least one person in the list.

### Notes

- The component is a client component (`'use client'`) because it relies on React state and step transitions.
- The layout centers content inside a `main` container with a `sm:max-w-md` width constraint.
- The completion action currently uses `alert("Split complete!")` and logs the final bill, but this can be expanded in the future to show a dedicated summary page or sharing options.
